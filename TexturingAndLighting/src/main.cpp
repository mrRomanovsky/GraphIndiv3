...
int main( int argc, char* argv[] )
{
	...
	g_MoonTexture = LoadTexture("../data/Textures/snow.jpg");
...
void DrawBallsRow(int z, int sz1, int sz2, int sz3) {
...
void DrawBallsVert(std::vector<int> ys, std::vector<float> sizes, std::vector<int> xs = std::vector<int>{ 0, 0 }, std::vector<int> zs = 
	...
	for (int i = 0; i < ys.size(); ++i) {
		...
		modelMatrix = glm::translate(glm::vec3(xs[i], ys[i], zs[i])) * glm::scale(glm::vec3(sizes[i]));
		...
		glUniform4fv(g_uniformMaterialEmissive, 10, glm::value_ptr(black));
		glUniform4fv(g_uniformMaterialDiffuse, 10, glm::value_ptr(white));
		glUniform4fv(g_uniformMaterialSpecular, 10, glm::value_ptr(white));
		glUniform1f(g_uniformMaterialShininess, 60.0f);
...
#pragma region snowman1
    // Draw the sun using a simple shader.
    glBindVertexArray(g_vaoSphere);

    glUseProgram( g_SimpleShaderProgram );
    glm::mat4 modelMatrix = glm::rotate( glm::radians(g_fSunRotation), glm::vec3(0,-1,0) ) * glm::translate(glm::vec3(0,0,18));
    glm::mat4 mvp = g_Camera.GetProjectionMatrix() * g_Camera.GetViewMatrix() * modelMatrix;
	...
	modelMatrix = glm::translate(glm::vec3(0, -15, 0)) * glm::scale(glm::vec3(12.756f));
	...
	DrawBallsVert(std::vector<int> {5, 19}, std::vector<float> {8.476f, 6.476f});

	// Draw eyes.
	//g_EarthTexture = LoadTexture("../data/Textures/eye.jpg");
	glBindTexture(GL_TEXTURE_2D, g_EarthTexture);

	modelMatrix = glm::translate(glm::vec3(2, 20, 5)) * glm::scale(glm::vec3(1.476f));
	mvp = g_Camera.GetProjectionMatrix() * g_Camera.GetViewMatrix() * modelMatrix;
	...
	modelMatrix = glm::translate(glm::vec3(0, 17, 5)) * glm::scale(glm::vec3(1.876f));
	mvp = g_Camera.GetProjectionMatrix() * g_Camera.GetViewMatrix() * modelMatrix;
...
#pragma region snowman2
	...
	//DrawBallsVert(std::vector<int> {5, 15}, std::vector<float> {6.476f, 4.476f}, std::vector<int>{-20, -20}, std::vector<int>{-20, -20});
	DrawBallsVert(std::vector<int> {-8, 5, 15}, std::vector<float> {8.476f, 6.476f, 4.476f}, std::vector<int>{-20, -20, -20}, std::vector<int>{-20, -20, -20});
	// Draw eyes.
	//g_EarthTexture = LoadTexture("../data/Textures/eye.jpg");
	glBindTexture(GL_TEXTURE_2D, g_EarthTexture);

	modelMatrix = glm::translate(glm::vec3(-17, 16, -13)) * glm::scale(glm::vec3(0.476f));
	mvp = g_Camera.GetProjectionMatrix() * g_Camera.GetViewMatrix() * modelMatrix;
	...
	modelMatrix = glm::translate(glm::vec3(-20, 16, -13)) * glm::scale(glm::vec3(0.476f));
	mvp = g_Camera.GetProjectionMatrix() * g_Camera.GetViewMatrix() * modelMatrix;
	...
	modelMatrix = glm::translate(glm::vec3(-19, 14, -15)) * glm::scale(glm::vec3(0.876f));
	mvp = g_Camera.GetProjectionMatrix() * g_Camera.GetViewMatrix() * modelMatrix;
